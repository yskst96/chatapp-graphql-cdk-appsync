...
const PREFIX = 'chatapp-graphql-cdk-appsync'

export class ChatappGraphqlCdkAppsyncStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props);


    // Cognito 
    const userPool = new UserPool(this, `${PREFIX}-chat-app-user-pool`, {
    ...
    new cdk.CfnOutput(this, `${PREFIX}-UserPoolId`, {
      value: userPool.userPoolId
    });

    new cdk.CfnOutput(this, `${PREFIX}-UserPoolClientId`, {
      value: userPoolClient.userPoolClientId
    });


    // DynamoDB
    const messageTable = new Table(this, `${PREFIX}-CDKMessageTable`, {
    ...
    const roomTable = new Table(this, `${PREFIX}-CDKRoomTable`, {
      billingMode: BillingMode.PAY_PER_REQUEST,
      partitionKey: {
        name: 'id',
        type: AttributeType.STRING,
    ...
    const messageTableServiceRole = new Role(this, `${PREFIX}-MessageTableServiceRole`, {
    ...
    const api = new GraphQLApi(this, `${PREFIX}-cdk-chat-app-api`, {
      name: "cdk-chat-app",
      logConfig: {
    ...
    // resolverを直書き
    // テンプレートから設定する方法もあり。　MappingTemplateクラスに生えてるstaticメソッドを使う
    //　https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-appsync.MappingTemplate.html
